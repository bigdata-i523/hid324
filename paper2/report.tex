...
Election is a formal decision-making process by a population to make a representation of them in a democratic system, this process of electing an an individual is called as Representative Democracy.
The election is one of the important activity in fulfillment of an individual's need, it is based on the fact that "Majority rule"\cite{1:online}, the theory holds good from cells in the human body to the transaction validations in Blockchain.  In the current digital world, this is still a slow, untrustworthy process. With all the implementation aspects, Big data and Blockchain can help modernizing the election process to more secure, trustworthy, foolproof, instant and decentralized process. In today's world the election results are impacted by big data why can't the election itself impacted by it for its own good.
...
\section{Election in India and U.S}
 ...
 U.S is one of the advanced countries in terms of election process, similar to any other democratic country, it has Federal election commission which will conduct elections, with 115 million voters and with 87.36\% internet penetration \cite{8:online}, data generated for the campaigning, voter validation, polling adds up to the big data use case. 
...
In the Big data terms, the voter selection can be synonyms with data ingestion into a data lake or no-SQL database. In technical terms, we have to persist not more than 1.2 billion records, considering the population of India. By efficiently sharding it per state, we can easily persist such volume of data into a distributed storage. This is already implemented in some of the countries like U.S 
in the name of SSN\cite{15:online} and in India, it is  Aadhaar ID\cite{13:online}.
We can easily read and filter out the voters based on their criminal records whether they are eligible for voting or contesting or not.


\subsection{Voting}

Voting, in simple terms, can be associated with aggregation/summing based on the key. Here the key is the contestant's symbol or name. By hosting this voting process in a website portal with API calls and load balancers, we can stream the votes and aggregate while it streamed to a persistent data store. With this approach, we can decentralize voting process. We can deter abusing this model by windowing the voting time. Upon completion of the window, we can reuse the infrastructure for other e-governance projects or we can reuse the e-governance infrastructure for this by using YARN as the resource manager, this can be possible even with streaming apps. 
...
Election frequency is proportional to the terms of the contestant for a given position. As the democratic principle believes that the people rule themselves, what if the representative after winning the election did not fulfill the expectations, people have to wait for the next term to make any change and it is not feasible financial wise. With the big data in place and resource being available, we can increase the frequency of election, so that the representative is accountable for the promises. For example, if we have a terms set for 5 years, every one year once, performance verification can be done in the form of a negative vote to the selected contestant and if the count is less than 50\% of his/her total vote count, the contestant can be disqualified.
...
For example, FollowMyVote proposes voting entirely on Blockchain. The anonymity of voter is maintained by Elliptic curve Cryptography\cite{16:online}, the transaction and consensus similar to Bitcoin network. And, BitCongress propose a system combining Bitcoin, Counterparty and Smart contracts. It proposes a token called VOTE, which can be transferred to the contestants and by the end of election, the token will be transferred back.
...
\section{Conclusion}
The election process is in need of dramatic modernization, though it is evolving in a pace different from the current world. To synchronize it with the current, there is a desperate need to implement this ancient system of democracy with the help of Big data and Blockchain. And by decentralizing it, the infrastructure of election can be utilized for other e-governance applications.
